...
GREG_LOGGER = logging.getLogger(__name__)
GREG_LOGGER.level = logging.INFO
GREG_HANDLER = logging.StreamHandler(stream=sys.stdout)
GREG_HANDLER.level = logging.INFO
GREG_LOGGER.addHandler(GREG_HANDLER)
GREG_HANDLER.setFormatter(fmt=logging.Formatter(
    '[%(asctime)s] [%(levelname)s] [%(name)s] %(message)s'))
CHATBOT_FILE_HANDLER = logging.FileHandler(filename="../greg_log.log")
CHATBOT_FILE_HANDLER.level = logging.INFO
GREG_LOGGER.addHandler(CHATBOT_FILE_HANDLER)
CHATBOT_FILE_HANDLER.setFormatter(fmt=logging.Formatter(
    '[%(asctime)s] [%(levelname)s] [%(name)s] %(message)s'))

if discord.version_info.major < 1:
    GREG_LOGGER.error("Your installed discord.py version is too low "
...
class GregContext(commands.Context):
...
class Greg(commands.Bot):
    """Botty things that are critical to Greg working"""
    ...
    def __init__(self, config, *args, **kwargs):
        ...
        if self.config['debug']:
            GREG_LOGGER.level = logging.DEBUG
            GREG_HANDLER.level = logging.DEBUG
    ...
    async def on_ready(self):
        ...
        GREG_LOGGER.info('Signed in as {}#{} ({})'.format(
        ...
        GREG_LOGGER.debug('Bot Invite: {}'.format(
    ...
    async def get_context(self, message, *, cls=GregContext):
    ...
    async def on_command_error(self, context, exception):
        if isinstance(exception, commands.NoPrivateMessage):
            ...
            if isinstance(context.channel, discord.TextChannel):
                GREG_LOGGER.error('Error in command <%d> (%d.name!r(%d.id) %d(%d.id) %d(%d.id) %d)',
                ...
                GREG_LOGGER.error('Error in command <%d> (DM %d(%d.id) %d)', context.command,
