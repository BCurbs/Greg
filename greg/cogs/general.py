...
class General(Cog):
    ...
    async def _help_all(self, ctx):
        ...
        info = discord.Embed(title='Greg: Info', description='A fun greg that learns as you talk to it. ',
                             color=discord.Color.blue())
        info.set_thumbnail(url=self.bot.user.avatar_url)
        info.add_field(name='About',
                       value="Greg: A fun bot.")
        ...
        await self._show_help(ctx, info, 'Greg: Commands', '', 'all commands', ctx.bot.commands)
    ...
    async def _help_command(self, ctx, command):
        ...
        info.set_footer(text='Greg Help | {!r} command | Info'.format(command.qualified_name))
    ...
    async def _show_help(self, ctx, start_page, title, description, footer, commands, **format_args):
        ...
        footer = 'Greg Help | {} | Page {}'.format(footer, '{page_num} of {len_pages}')
